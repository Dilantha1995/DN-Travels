...
// Read a File/Blob (image) as a base64 string (no data: prefix).
...
// Send an image (screenshot) to the AI parser. Works for booking confirmations,
// search-result pages, e-tickets — anything the AI can read visually.
export async function aiParseImage(file) {
  ...
  if (!res.ok) {
    let detail = "";
    try { detail = (await res.json()).detail || ""; } catch {}
    throw new Error(`AI image parse failed (${res.status}). ${detail}`);
  }
  return res.json();
}
